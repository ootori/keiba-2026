...
| 特徴量カラム選定 | train_df.columnsから決定 | train_df ∩ valid_df の共通カラムのみ使用 | parquet再構築タイミング差でtrain/validのカラムが不整合になる場合の安全策。不整合時はWARNINGログで通知 |

### 改善提案の実装状況

`docs/new_feature.md` の改善提案に基づく実装。
