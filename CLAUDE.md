...
2. **過去成績(17):** 着順, 勝率, 連対率, 複勝率（全走/直近N走）, 2着率, exacta傾向, 1着とのタイム差平均
...
11. **調教師(14):** 勝率, 複勝率, 場別成績, 騎手×調教師コンビ, 芝ダ別勝率/複勝率, 距離帯別勝率, 馬場別勝率, 直近30日勝率, 重賞複勝率, 休み明け複勝率
...
14b. **フォームモメンタム(6):** 着順トレンド傾斜, 最終勝利日数, 連続3着以内, 前走vsベスト差, 改善フラグ, 昇級初戦フラグ
15. **オッズ(7):** 単勝オッズ, 複勝オッズ, 人気順（※予測タイミング依存）
16. **クロス特徴量(10):** 距離変更, 芝ダ変更, クラス変更, 斤量/体重比, 前走牝馬限定フラグ, 現在レース牝馬限定フラグ
16b. **ペース構造(5):** 逃げ先行馬数, 予想ペース, 脚質×ペース相性, レース平均スピード指数, レース最大スピード指数
17. **レース内相対特徴量(40):** 主要能力指標のレース内Zスコア・ランク（血統4指標+2着率関連2指標追加）
...
合計 **約256特徴量**（v1: ~182 + マイニング: 7 + BMS条件別: 6 + レーティング: 28 + 調教師条件別: 7 + フォームモメンタム: 6 + 2着固有: 4 + ペース構造: 5 + 相対特徴量追加分: 12 ※サプリメント含む。詳細は `docs/feature_design.md` 参照）
...
| A: 調教師条件別 | +7 | jockey_trainer.py | **実装済み** | 芝ダ別/距離帯別/馬場別/直近30日/重賞/休み明け |
| D: フォームモメンタム | +6 | horse.py | **実装済み** | 着順トレンド傾斜/最終勝利日数/連続3着以内/ピーク比較/改善フラグ/昇級初戦 |
| E: ペース構造 | +5 | pipeline.py | **実装済み** | 逃げ先行馬数/予想ペース/脚質×ペース相性/レースレベル |
| 2着固有特徴量 | +4 | horse.py | **実装済み** | 2着率/exacta傾向/1着とのタイム差平均 |
| 2着相対特徴量 | +4 | pipeline.py | **実装済み** | 2着率/exacta傾向のZスコア+ランク |
...
# LambdaRank + 1着重み寄せモード（単勝回収率重視）
python run_train.py --ranking --relevance-mode win --train-only --model-name ranking_win
...
# === LambdaRank 確率変換方式 ===
# デフォルト: softmax（従来方式）
python run_train.py --eval-only --model-name ranking_model --prob-method softmax

# Plackett-Luce 変換（softmaxと数学的に同一だが理論的根拠が明確）
python run_train.py --eval-only --model-name ranking_model --prob-method plackett_luce

# Gumbel モンテカルロ（スコアにGumbelノイズを加えてMCサンプリング）
python run_train.py --eval-only --model-name ranking_model --prob-method gumbel_mc

# 本番予測で Plackett-Luce を使用
python run_predict.py --year 2025 --monthday 0622 --all-day --prob-method plackett_luce
...
- **value_bet のモデルタイプ別正規化:** `value_bet` 戦略は `target_type` と `ranking` に応じて適切な確率変換を適用する:
  ...
  - `ranking=True`: `--prob-method` に応じた確率変換（デフォルト: softmax）
    - `softmax`: exp(s_i) / Σexp(s_j)（従来方式）
    - `plackett_luce`: Plackett-Luce モデルに基づく1着確率（数学的にはsoftmaxと同一だが、逐次選択モデルの理論的裏付けあり）
    - `gumbel_mc`: スコアにGumbel(0,1)ノイズを加えてモンテカルロサンプリング（10000回）で1着確率を推定。過度に集中した確率分布を緩和し、僅差の馬間に揺らぎを与える
